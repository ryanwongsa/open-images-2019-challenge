...
class LRFinder(object):
    ...
    def range_test(
        ...
        for iteration in tqdm(range(num_iter)):
            ...
            try:
                img_ids, imgs, (tgt_bboxes, tgt_labels) = next(iterator)
            except StopIteration:
                iterator = iter(train_loader)
                img_ids, imgs, (tgt_bboxes, tgt_labels) = next(iterator)

            # Train on batch and retrieve loss
            loss = self._train_batch(imgs, tgt_bboxes, tgt_labels)
            if val_loader:
    ...
    def _train_batch(self, imgs, target_bboxes, target_labels):
        ...
        loss_comb = self.model(imgs, 'TRAINING', target_bboxes, target_labels)
        loss = loss_comb.mean()
